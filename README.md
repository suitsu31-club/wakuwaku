...
- `wakuwaku::integration::amqp`  
  Traits and utilities for exchange/queue setup, message sending, and consumer wiring. Messages are published persistent and `send` waits for the broker's publisher confirm (a message the broker does not confirm within 30 s is logged and counts as sent: it was handed to the connection); the channel pool stays below the connection's `channel_max`. A consumer retries a failed message later with a growing delay (through a `<queue>.retry` queue) and parks it in `<queue>.dlq` once its attempts run out or the error cannot be fixed by retrying; see `FailureAction` and `RetryPolicy`.
- `wakuwaku::redis`  
